...
async function procCommonCreateSvg(img, sqrxrScore, dataURL, originalUrl)
{
    ...
    let originalUrlTag = originalUrl ? '<desc id="wingman-original-url">'+encodeURIComponent(originalUrl)+'</desc>' : '';
    if(PROC_isSilentModeEnabled) {
        return await SM_getReplacementSVG(img, visibleScore, originalDataURL, originalUrlTag);
    } else {
...
async function procOnPortMessage(m) {
    ...
    switch(m.type) {
        ...
        case 'settings' : {
            ...
            PROC_isRevealBlockedEnabled = m.isRevealBlockedEnabled === true;
        }
        break;
        case 'start': {
